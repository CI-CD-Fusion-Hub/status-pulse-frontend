...
<template>
  ...
  <VModal v-model:isActive="isAddModalVissible" header="Add Widget" button-label="Add Widget" @on-send="addData" @on-close="closeModal">
    ...
    <template v-if="formData.endpoints">
      <VToggle v-model:data="formData.type" type="icon" label="Widget type" :options="[{ icon: 'bx bx-bar-chart-alt', label: 'Uptime bar', value: 'Uptime' }, { icon: 'bx bx-line-chart', label: 'Line Chart', value: 'LineChart' }]" option-label="label" option-value="value" option-icon="icon" />
